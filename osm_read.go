...
func ReadOSMFromFileMultipart(bufReader multipart.File, parserType ParserType, allowedAgentTypes []types.AgentType) (*OSMWaysNodes, error) {
	/* Process ways */
	if VERBOSE {
		log.Info().Str("scope", "osm_read").Msg("Processing ways")
	}
	...
	nodesSeen := make(map[osm.NodeID]struct{})
	{
		...
		case PARSER_XML:
			scannerWays = osmxml.New(context.Background(), bufReader)
		case PARSER_PBF:
			scannerWays = osmpbf.New(context.Background(), bufReader, 4)
		default:
			return nil, fmt.Errorf("file extension '%s' is not handled yet", parserType)
		}
		defer scannerWays.Close()
	...
	nodes := make(map[osm.NodeID]*wrappers.NodeOSM)
	{
		...
		switch parserType {
		case PARSER_XML:
			scannerNodes = osmxml.New(context.Background(), bufReader)
		case PARSER_PBF:
			scannerNodes = osmpbf.New(context.Background(), bufReader, 4)
		default:
			return nil, fmt.Errorf("file extension '%s' is not handled yet", parserType)
		}
		defer scannerNodes.Close()
